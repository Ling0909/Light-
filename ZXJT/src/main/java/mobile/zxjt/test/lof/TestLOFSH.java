 ...
 * LOF赎回测试
 */
public class TestLOFSH extends TestBase {
	private PageLOF mPage = PageManager.getPage(PageLOF.class);

	@BeforeClass
	public void enterLOFSH() {
		Navigator.navigate("LOF基金赎回", mPage);
	}

	@Test(dataProvider = DataProviderX.NAME_LAZY, dataProviderClass = DataProviderX.class)
	public void testLOFSH(@Mapper(InfoMapper.class) Info info, CustomRecord record) {
		// 输入代码并校验名称
