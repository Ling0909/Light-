 ...
 * LOF认购
 */
public class TestLOFRG extends TestBase {
	private PageLOF mPage = PageManager.getPage(PageLOF.class);

	@BeforeClass
	public void enterLOFRG() {
		Navigator.navigate("LOF基金认购", mPage);
	}

	@Test(dataProvider = DataProviderX.NAME_LAZY, dataProviderClass = DataProviderX.class)
	public void testLOFRG(@Mapper(InfoMapper.class) Info info, CustomRecord record) {
		// 输入代码并校验名称
