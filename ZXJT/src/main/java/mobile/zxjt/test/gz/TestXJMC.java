 ...
 * 限价卖出测试
 */
public class TestXJMC extends PageBase {
	private PageGFZR mPage = PageManager.getPage(PageGFZR.class);

	@BeforeClass
	public void enterXJMC() {
		Navigator.navigate("限价卖出", mPage);
	}

	@Test(dataProvider = DataProviderX.NAME_LAZY, dataProviderClass = DataProviderX.class)
	public void testXJMC(@Mapper(InfoMapper.class) Info info, CustomRecord record) {
		String vNameToSelect = info.getName();
		if ("长 白 5".equals(vNameToSelect)) {
