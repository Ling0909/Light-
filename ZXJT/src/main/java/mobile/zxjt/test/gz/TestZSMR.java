 ...
 * 做市买入测试
 */
public class TestZSMR extends PageBase {
	private PageGFZR mPage = PageManager.getPage(PageGFZR.class);

	@BeforeClass
	public void enterZSMR() {
		Navigator.navigate("做市买入", mPage);
	}

	@Test(dataProvider = DataProviderX.NAME_LAZY, dataProviderClass = DataProviderX.class)
	public void testZSMR(@Mapper(InfoMapper.class) Info info, CustomRecord record) {
