 ...
 * 定价买入测试
 */
public class TestDJMR extends PageBase {
	private PageGFZR mPage = PageManager.getPage(PageGFZR.class);

	@BeforeClass
	public void enterDJMR() {
		Navigator.navigate("定价买入", mPage);
	}

	@Test(dataProvider = DataProviderX.NAME_LAZY, dataProviderClass = DataProviderX.class)
	public void testDJMR(@Mapper(InfoMapper.class) Info info, CustomRecord record) {
