 ...
 * 定价卖出测试
 */
public class TestDJMC extends PageBase {
	private PageGFZR mPage = PageManager.getPage(PageGFZR.class);

	@BeforeClass
	public void enterDJMC() {
		Navigator.navigate("定价卖出", mPage);
	}

	@Test(dataProvider = DataProviderX.NAME_LAZY, dataProviderClass = DataProviderX.class)
	public void testDJMC(@Mapper(InfoMapper.class) Info info, CustomRecord record) {
		String vName = mPage.doInputCode(info.getCode(), info.getName(), false);
		Assertions.assertThat(vName).as("校验名称").isEqualToIgnoringWhitespace(info.getName());
		// 输入数量
