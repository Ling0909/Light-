 ...
 * 限价买入测试
 */
public class TestXJMR extends PageBase {
	private PageGFZR mPage = PageManager.getPage(PageGFZR.class);

	@BeforeClass
	public void enterXJMR() {
		Navigator.navigate("限价买入", mPage);
	}

	@Test(dataProvider = DataProviderX.NAME_LAZY, dataProviderClass = DataProviderX.class)
	public void testXJMR(@Mapper(InfoMapper.class) Info info, CustomRecord record) {
