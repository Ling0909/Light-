 ...
 * 港股通额度查询测试
 */
public class TestHKEDCX extends TestBase {
	private PageHKFZ_EDCX mPage = PageManager.getPage(PageHKFZ_EDCX.class);

	@BeforeClass
	public void enterHKEDCX() {
		Navigator.navigate("额度查询", mPage);
	}

	@Test(dataProvider = DataProviderX.NAME_LAZY, dataProviderClass = DataProviderX.class)
	public void testHKEDCX(@Key("沪港通") String hgt, @Key("深港通") String sgt, CustomRecord record) {
