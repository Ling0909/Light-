 ...
 * ETF网下现金认购测试
 */
public class TestETFWXXJRG extends TestBase {
	private PageETF mPage = PageManager.getPage(PageETF.class);

	@BeforeClass
	public void enterETFWXXJRG() {
		Navigator.navigate("网下现金认购", mPage);
	}

	@Test(dataProvider = DataProviderX.NAME_LAZY, dataProviderClass = DataProviderX.class)
	public void testETFWXXJRG(@Mapper(InfoMapper.class) Info info, CustomRecord record) {
		// 输入代码并校验名称
